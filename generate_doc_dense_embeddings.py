...
def gen_ctx_vectors(
    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        for ctx in ctx_rows[batch_start: batch_start + bsz]:
            token_tensors = tensorizer.text_to_tensor(ctx[1], title_list=", ".join(
                ctx["title_list"].split(" # ")) if "title_list" in ctx else None,
        ...
        ctx_seg_batch = move_to_device(
            torch.zeros_like(ctx_ids_batch), args.device)
...
def main(args):
    ...
    with open(args.ctx_file) as tsvfile:
        ...
        for row in reader:
            if row[0] != "id":
                if len(rows) == 3:
                    rows.extend([(row[0], row[1], row[2])])
                elif len(row) == 4:
                    rows.extend([(row[0], row[1], row[2], " ".join(row[3].split(" # ")))])
    shard_size = int(len(rows) / args.num_shards)
    if args.shard_id != shard_size - 1:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu_id", type=str, default="0", help="The gpu id to use"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
