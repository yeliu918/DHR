...
def validate(
    ...
    logger.info("Top-1   Top-5   Top-20   Top-100 is:   ", top_k_hits[0], top_k_hits[4], top_k_hits[19], top_k_hits[99])
...
def validate_doc(
    ...
    top_k_hits = [v / len(result_ctx_ids) for v in top_k_hits]
    logger.info("Validation results: top k documents hits accuracy %s", top_k_hits)
    print("Top-1  %s   Top-5   %s   Top-20   %s   Top-100   %s   is:   ", top_k_hits[0], top_k_hits[4], top_k_hits[19],
          top_k_hits[99])
    return match_stats.questions_doc_hits
...
def main(args):
    ...
    logger.info("Finish Index the top document............")
    all_documents, id_title = load_documents(args.docs_file)
    logger.info("Finish Loading the whole documents............")
    title_text = load_passages(args.ctx_file)
    logger.info("Finish Loading the whole passages............")
    ...
    args.qa_file = args.qa_file.split(",")
    args.out_file = args.out_file.split(",")
    for infile, outfile in zip(args.qa_file, args.out_file):
        for ds_item in parse_qa_csv_file(infile):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="All docs file in the tsv format: id \\t docs_intro \\t title",
    )
    parser.add_argument(
        "--n_docs",
        type=int,
        default=100,
        help="The number of document to retrieve",
    )
    parser.add_argument(
        "--encoded_ctx_file",
        type=str,
