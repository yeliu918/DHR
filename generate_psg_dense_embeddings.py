    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        batch_token_tensors = [
            tensorizer.text_to_tensor(ctx[1],
                                      title_list=", ".join(ctx[3].split(" # ")) if args.contain_title_list else None,
...
def main(args):
    ...
    # if "embeddings.position_ids" in ctx_state:
    #     ctx_state.pop("embeddings.position_ids")
        ...
        for row in reader:
            if row[0] != "id":
                if len(row) == 3:
                    ...
                    rows.extend([(row[0], row[1], row[2], row[3])])
