...
"""
 Pipeline to train dhr Biencoder
"""
...
from dhr.models.biencoder_doc import BiEncoder, BiEncoderNllLoss, BiEncoderBatch
...
from dhr.utils.data_utils import (
    ShardedDataIterator,
    read_data_from_json_files,
    Tensorizer,
)
from dhr.utils.dist_utils import all_gather_list
...
class BiEncoderTrainer(object):
    ...
    provided pools of negative passages. For full IR accuracy evaluation, please see generate_psg_dense_embeddings.py
    ...
    def validate_nll(self) -> float:
        ...
        num_hard_from_psgs_negatives = args.from_psgs_negatives
        num_hard_dhr_negatives = args.dhr_negatives
        contain_title_list = args.contain_title_list
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            biencoder_input = BiEncoder.create_biencoder_input(
                samples_batch,
                self.tensorizer,
                args.insert_title_list,
                num_hard_negatives,
                num_other_negatives,
                num_hard_from_psgs_negatives,
                num_hard_dhr_negatives,
                contain_title_list,
                shuffle=False,
            )
    ...
    def validate_average_rank(self) -> float:
        ...
        num_hard_from_psgs_negatives = args.val_av_rank_from_psgs_neg
        num_hard_dhr_negatives = args.val_av_rank_dhr_neg
        contain_title_list = args.contain_title_list
        log_result_step = args.log_batch_step
...
def main():
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="amount of 'in section' negative ctx per question",
    )
    parser.add_argument(
        "--dhr_negatives",
        default=1,
        type=int,
        help="amount of 'dhr' negative ctx per question",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--contain_title_list",
        action="store_true",
        help="Whether to use title list in the context",
    )
    parser.add_argument(
        "--insert_title_list",
        action='store_true',
        help="Whether insert the title list in doc representation",
    )
    parser.add_argument(
        "--val_av_rank_bsz",
        type=int,
