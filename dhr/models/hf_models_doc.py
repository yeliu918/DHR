...
from .biencoder_doc import BiEncoder
...
def get_bert_biencoder_components(args, inference_only: bool = False, **kwargs):
    ...
    fix_q_encoder = (
        args.fix_q_encoder if hasattr(args, "fix_q_encoder") else False
    )

    biencoder = BiEncoder(
        question_encoder, ctx_encoder, fix_ctx_encoder=fix_ctx_encoder, fix_q_encoder=fix_q_encoder
    )

    biencoder.ctx_model.training = True
    biencoder.question_model.training = True
    # for nm, param in biencoder.ctx_model.named_parameters():
    #     if "encoder" in nm:
    #         param.requires_grad = False
    # for nm, param in biencoder.question_model.named_parameters():
    #     if "encoder" in nm:
    #         param.requires_grad = False
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                ...
                if not any(nd in n for nd in no_decay)  # and not any(nd in n for nd in ["encoder"])
            ],
        ...
        },
        {
            "params": [
                p
                for n, p in model.named_parameters()
                if any(nd in n for nd in no_decay)  # and not any(nd in n for nd in ["encoder"])
            ],
            "weight_decay": 0.0,
        },
    ]
...
class BertTensorizer(Tensorizer):
    ...
    def text_to_tensor(
            self, text: str, title: str = None, title_list=None, add_special_tokens: bool = True
    ):
        text = text.strip()
