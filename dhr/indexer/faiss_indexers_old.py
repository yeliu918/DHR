...
class DenseIndexer(object):
    ...
    def index_data_with_restricted(self, buffer: List):
        self.index.reset()
        if 0 < self.buffer_size == len(buffer):
            self._new_index_batch(buffer)
        self._new_index_batch(buffer)
    ...
    def _new_index_batch(self, data: List[Tuple[object, np.array]]):
        raise NotImplementedError
    ...
    def _id_mapping(self, db_ids: List):
        self.index_id_to_db_id = db_ids
