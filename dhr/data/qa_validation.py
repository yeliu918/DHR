...
def calculate_doc_matches(
    id_title: Dict[object, str],
    title_text: Dict[object, str],
    ...
    """
    global dpr_title_text
    dpr_title_text=title_text
    global dpr_id_title
    dpr_id_title = id_title
    ...
    get_score_partial = partial(
        check_doc_answer, match_type=match_type, tokenizer=tokenizer,
    )
...
def check_doc_answer(questions_answers_docs, tokenizer, match_type) -> List[bool]:
    ...
    for i, doc_id in enumerate(doc_ids):
        doc_title = dpr_id_title[doc_id]
        text = " ".join(dpr_title_text[doc_title])
...
def calculate_psgs_doc_matches(
    title_text: Dict[object, str],
    answers: List[List[str]],
    closest_docs: List[Tuple[List[object], List[float]]],
    workers_num: int,
    ...
    get_score_partial = partial(
        check_psgs_in_doc_answer, match_type=match_type, tokenizer=tokenizer,
    )
...
def check_psgs_in_doc_answer(questions_answers_docs, tokenizer, match_type) -> List[bool]:
    """Search through all the top docs to see if they have any of the answers."""
    answers, doc_titles = questions_answers_docs
...
def calculate_matches(
    ...
    logger.info("Matching answers in top passages...")
...
def exact_match_score(prediction, ground_truth, match_type = "string"):
    if match_type == "string":
        return _normalize_answer(prediction) == _normalize_answer(ground_truth)
    elif match_type == "regex":
        return regex_match(_normalize_answer(prediction), _normalize(ground_truth))
