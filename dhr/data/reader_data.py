    ...
    def create_reader_sample_ids(sample: ReaderPassage, question: str):
        ...
        if sample.passage_token_ids is None:
            sample.passage_token_ids = tensorizer.text_to_tensor(
                sample.passage_text, title_list = sample.title_list, add_special_tokens=False
            )
...
def _select_reader_passages(
    ...
    def find_answer_spans(ctx: ReaderPassage, match):
        if ctx.has_answer:
            ...
            answer_spans = [
                _find_answer_positions(ctx.passage_token_ids, answers_token_ids[i], match)
    ...
    selected_positive_ctxs = list(
        filter(
            ...
            [find_answer_spans(ctx, match) for ctx in positive_ctxs_from_gold_page],
        )
    )
    ...
    if not selected_positive_ctxs:  # fallback to positive ctx not from gold pages
        selected_positive_ctxs = list(
            filter(
                lambda ctx: ctx.has_answer,
                [find_answer_spans(ctx, match) for ctx in positive_samples],
            )
        )[0:max_positives]
    ...
    if include_gold_passage and question in gold_passage_map:
        ...
        if not included_gold_passage:
            gold_passage = find_answer_spans(gold_passage, match)
...
# ToDo:
def _find_answer_positions(ctx_ids: T, answer: T, match) -> List[Tuple[int, int]]:
    ...
    for i in range(0, c_len - a_len + 1):
        if match == "string":
            ...
            if regex_match(answer, ctx_ids[i : i + a_len]):
                answer_occurences.append((i, i + a_len - 1))
